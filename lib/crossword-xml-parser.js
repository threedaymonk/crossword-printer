...
const parseCrosswordXml = (xml) => {
  ...
  const pMetadata = doc.get("//p:metadata", NAMESPACES);
  const clues = parseClues(pCrossword);
  const grid = parseGrid(pGrid);
  const metadata = parseMetadata(pMetadata);
  return {
    ...grid,
    ...clues,
    ...metadata
...
const parseMetadata = (pMetadata) => {
  return take({})
    .pipe(mergeCond, "creator", pMetadata.get("p:creator", NAMESPACES), textContent)
    .result();
};
